...
static void update_progress_bar(long downloaded, long total_size) {
    ...
    if (total_size <= 0 || downloaded < 0) return;
    ...
    printf("%s%s[%3d%%]%s %s|", CLEAR_LINE, COLOR_CYAN, percent, COLOR_RESET, COLOR_GREEN);
    for (int i = 0; i < bar_width; ++i) {
        putchar(i < filled ? '=' : (i == filled ? '>' : ' '));
    }
    printf("|%s (%ld/%ld MB)", COLOR_RESET, downloaded / (1024 * 1024), total_size / (1024 * 1024));
    fflush(stdout);
...
static int fetch_http_header(int fd, char *buf, int buf_len) {
    ...
    while ((nbytes = recv(fd, tmp, 1, 0)) == 1) {
        if (offset >= buf_len - 1) return buf_len;
...
int downloader(const char *url, const char *filename) {
    ...
    if ((sockfd = connect_to_url(url)) < 0) return -1;
