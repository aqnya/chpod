...
static int connect_to_url_part(const char *url, long start, long end,
                               int is_probe, int *status_code) {
  int cfd;
...
void *download_part(void *arg) {
  ...
  sockfd = connect_to_url_part(data->url, data->start_byte, data->end_byte, 0,
                               &status_code);
  ...
  while (1) {
    ...
    update_progress_bar(atomic_load(data->total_downloaded),
                        resp.content_length);
  }
...
int downloader(const char *url, const char *filename) {
  ...
  for (int i = 0; i < num_threads; i++) {
    ...
    thread_data[i].end_byte = (i == num_threads - 1) ? resp.content_length - 1
                                                     : (i + 1) * part_size - 1;
